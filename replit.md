...
**November 9, 2025**
- **FIXED: Invoice Generation "No Order Selected" Error**: Fixed critical bug where clicking "Generate Invoice" from the orders page resulted in a "no order selected" error. The issue was that query parameters weren't being read correctly from Wouter's location string. Now properly parses the orderId from the Wouter location path instead of window.location.search.
